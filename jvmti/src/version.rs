...
impl VersionNumber {
    ...
    pub fn from_u32(version: &u32) -> VersionNumber {
        let major_version =
            ((version & JVMTI_VERSION_MASK_MAJOR) >> JVMTI_VERSION_SHIFT_MAJOR) as u16;
        let minor_version =
            ((version & JVMTI_VERSION_MASK_MINOR) >> JVMTI_VERSION_SHIFT_MINOR) as u8;
        let micro_version =
            ((version & JVMTI_VERSION_MASK_MICRO) >> JVMTI_VERSION_SHIFT_MICRO) as u8;
