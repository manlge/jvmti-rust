...
impl Agent {
    ...
    pub fn new(vm: JavaVMPtr) -> Agent {
        ...
        match jvm_agent.get_environment() {
            ...
            Err(err) => panic!(
                "FATAL: Could not get JVMTI environment: {}",
                translate_error(&err)
            ),
        }
    }
        ...
        match self.environment.add_capabilities(&self.capabilities) {
            Ok(caps) => {
                ...
                match self.environment.set_event_callbacks(self.callbacks.clone()) {
                    None => {
                        self.environment.set_event_notification_mode(
                            VMEvent::VMObjectAlloc,
                            self.callbacks.vm_object_alloc.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::VMObjectFree,
                            self.callbacks.vm_object_free.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::VMStart,
                            self.callbacks.vm_start.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::VMInit,
                            self.callbacks.vm_init.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::VMDeath,
                            self.callbacks.vm_death.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::MethodEntry,
                            self.callbacks.method_entry.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::MethodExit,
                            self.callbacks.method_exit.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::ThreadStart,
                            self.callbacks.thread_start.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::ThreadEnd,
                            self.callbacks.thread_end.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::Exception,
                            self.callbacks.exception.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::ExceptionCatch,
                            self.callbacks.exception_catch.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::MonitorWait,
                            self.callbacks.monitor_wait.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::MonitorWaited,
                            self.callbacks.monitor_waited.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::MonitorContendedEnter,
                            self.callbacks.monitor_contended_enter.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::MonitorContendedEntered,
                            self.callbacks.monitor_contended_entered.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::FieldAccess,
                            self.callbacks.field_access.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::FieldModification,
                            self.callbacks.field_modification.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::GarbageCollectionStart,
                            self.callbacks.garbage_collection_start.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::GarbageCollectionFinish,
                            self.callbacks.garbage_collection_finish.is_some(),
                        );
                        self.environment.set_event_notification_mode(
                            VMEvent::ClassFileLoadHook,
                            self.callbacks.class_file_load_hook.is_some(),
                        );
                    }
                    Some(error) => {
                        println!("Couldn't register callbacks: {}", translate_error(&error))
                    }
                }
            }
    ...
    pub fn on_monitor_wait(&mut self, handler: Option<FnMonitorWait>) {
        ...
        let has_some = handler
            ...
            .or(self.callbacks.monitor_contended_entered)
            .is_some();
    ...
    pub fn on_garbage_collection_start(&mut self, handler: Option<FnGarbageCollectionStart>) {
        ...
        self.capabilities.can_generate_garbage_collection_events = handler
            .or(self.callbacks.garbage_collection_finish)
            .is_some();
    }
    ...
    pub fn on_garbage_collection_finish(&mut self, handler: Option<FnGarbageCollectionFinish>) {
        ...
        self.capabilities.can_generate_garbage_collection_events = handler
            .or(self.callbacks.garbage_collection_start)
            .is_some();
    }
