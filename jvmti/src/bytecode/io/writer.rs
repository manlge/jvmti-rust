...
impl<'a> ClassWriter<'a> {
    pub fn new<T>(target: &'a mut T) -> ClassWriter
    where
        T: Write,
    {
    ...
    pub fn write_constant_pool(&mut self, cp: &ConstantPool) -> Result<usize, Error> {
        cp.constants
            .iter()
            .fold(self.write_u16(cp.cp_len() as u16), |acc, x| match acc {
    ...
    fn write_constant(&mut self, constant: &Constant) -> Result<usize, Error> {
        match constant {
            &Constant::Utf8(ref bytes) => self
                .write_u8(1)
                .and(self.write_u16(bytes.len() as u16))
                .and(self.write_n(bytes)),
            ...
            &Constant::FieldRef {
                class_index: ref c_idx,
                name_and_type_index: ref n_idx,
            } => self
                .write_u8(9)
                .and(self.write_u16(c_idx.idx as u16))
                .and(self.write_u16(n_idx.idx as u16)),
            &Constant::MethodRef {
                class_index: ref c_idx,
                name_and_type_index: ref n_idx,
            } => self
            ...
            &Constant::NameAndType {
                name_index: ref n_idx,
                descriptor_index: ref d_idx,
            } => self
                .write_u8(12)
                .and(self.write_u16(n_idx.idx as u16))
                .and(self.write_u16(d_idx.idx as u16)),
            &Constant::MethodHandle {
                reference_kind: ref kind,
                reference_index: ref r_idx,
            } => self
                .write_u8(15)
                .and(self.write_u8(kind.to_u8()))
                .and(self.write_u16(r_idx.idx as u16)),
            &Constant::InvokeDynamic {
                bootstrap_method_attr_index: ref m_idx,
                name_and_type_index: ref n_idx,
            } => self
                .write_u8(18)
                .and(self.write_u16(m_idx.idx as u16))
                .and(self.write_u16(n_idx.idx as u16)),
            &Constant::Placeholder => Ok(0),
            _ => Err(Error::new(
                ErrorKind::InvalidData,
                "Unknown constant detected",
            )),
        }
    }
    ...
    fn write_interfaces(&mut self, ifs: &Vec<ConstantPoolIndex>) -> Result<usize, Error> {
        ifs.iter()
            .fold(self.write_u16(ifs.len() as u16), |acc, x| match acc {
    ...
    fn write_fields(&mut self, fields: &Vec<Field>, cp: &ConstantPool) -> Result<usize, Error> {
        fields
            .iter()
            .fold(self.write_u16(fields.len() as u16), |acc, x| match acc {
    ...
    fn write_methods(&mut self, methods: &Vec<Method>, cp: &ConstantPool) -> Result<usize, Error> {
        methods
            .iter()
            .fold(self.write_u16(methods.len() as u16), |acc, x| match acc {
    ...
    fn write_attributes(
        ...
        attributes.iter().fold(
            self.write_u16(attributes.len() as u16),
            |acc, x| match acc {
    ...
    fn write_attribute(
        ...
        match attribute {
            &Attribute::RawAttribute {
                ...
                .write_u16(n_idx.idx as u16)
                .and(self.write_u32(bytes.len() as u32))
                .and(self.write_n(bytes)),
            &Attribute::ConstantValue(ref idx) => self
                .write_u16(cp.get_utf8_index("ConstantValue") as u16)
                .and(self.write_u32(2))
                .and(self.write_u16(idx.idx as u16)),
            &Attribute::Code {
                max_stack,
                max_locals,
                ref code,
            ...
            &Attribute::Exceptions(ref table) => self
                .write_u16(cp.get_utf8_index("Exceptions") as u16)
                .and(self.write_u32(2 + (table.len() as u32) * 2))
                .and(self.write_u16(table.len() as u16))
                .and(
                    table
                        .iter()
                        .fold(Ok(0), |_, x| self.write_u16(x.idx as u16)),
                ),
            &Attribute::InnerClasses(ref table) => self
                .write_u16(cp.get_utf8_index("InnerClasses") as u16)
                .and(self.write_u32(2 + (table.len() as u32) * 8))
                .and(self.write_u16(table.len() as u16))
            ...
            &Attribute::EnclosingMethod {
                ...
                .write_u16(cp.get_utf8_index("EnclosingMethod") as u16)
                .and(self.write_u32(4))
                .and(self.write_u16(class_index.idx as u16))
                .and(self.write_u16(method_index.idx as u16)),
            &Attribute::Synthetic => self
                .write_u16(cp.get_utf8_index("Synthetic") as u16)
                .and(self.write_u32(0)),
            &Attribute::Signature(ref idx) => self
                .write_u16(cp.get_utf8_index("Signature") as u16)
                .and(self.write_u32(2))
                .and(self.write_u16(idx.idx as u16)),
            &Attribute::SourceFile(ref idx) => self
                .write_u16(cp.get_utf8_index("SourceFile") as u16)
                .and(self.write_u32(2))
                .and(self.write_u16(idx.idx as u16)),
            &Attribute::SourceDebugExtension(ref vec) => self
                .write_u16(cp.get_utf8_index("SourceDebugExtension") as u16)
                .and(self.write_u32(vec.len() as u32))
                .and(self.write_n(vec)),
            &Attribute::LineNumberTable(ref table) => self
                .write_u16(cp.get_utf8_index("LineNumberTable") as u16)
                .and(self.write_u32(2 + table.len() as u32 * 4))
                .and(self.write_u16(table.len() as u16))
                .and(table.iter().fold(Ok(0), |_, x| {
                    self.write_u16(x.start_pc)
                        .and(self.write_u16(x.line_number))
                })),
            &Attribute::LocalVariableTable(ref table) => self
                .write_u16(cp.get_utf8_index("LocalVariableTable") as u16)
                .and(self.write_u32(2 + table.len() as u32 * 10))
                .and(self.write_u16(table.len() as u16))
                .and(table.iter().fold(Ok(0), |_, x| {
                    self.write_u16(x.start_pc)
            ...
            &Attribute::LocalVariableTypeTable(ref table) => self
                .write_u16(cp.get_utf8_index("LocalVariableTypeTable") as u16)
                .and(self.write_u32(2 + table.len() as u32 * 10))
                .and(self.write_u16(table.len() as u16))
                .and(table.iter().fold(Ok(0), |_, x| {
                    self.write_u16(x.start_pc)
            ...
            &Attribute::Deprecated => self
                .write_u16(cp.get_utf8_index("Deprecated") as u16)
                .and(self.write_u32(0)),
            &Attribute::RuntimeVisibleAnnotations(ref table) => {
                self.write_u16(cp.get_utf8_index("RuntimeVisibleAnnotations") as u16)
                    ...
                    .and(
                        table
                            .iter()
                            .fold(Ok(0), |_, x| self.write_annotation(x, cp)),
                    )
            }
            ...
            &Attribute::RuntimeVisibleParameterAnnotations(ref table) => {
                self.write_u16(cp.get_utf8_index("RuntimeVisibleParameterAnnotations") as u16)
                    // attribute_length
                    .and(self.write_u32(table.iter().fold(1, |acc, x| {
                        acc + x.iter().fold(2, |acc2, x2| acc2 + x2.len()) as u32
                    })))
                    // num_parameters
                    .and(self.write_u8(table.len() as u8))
                    // parameter_annotations
                    .and(table.iter().fold(Ok(0), |_, ann_table| {
                        self.write_u16(ann_table.len() as u16).and(
                            ann_table
                                .iter()
                                .fold(Ok(0), |_, ann| self.write_annotation(ann, cp)),
                        )
                    }))
            }
            ...
            &Attribute::RuntimeVisibleTypeAnnotations(ref table) => {
                self.write_u16(cp.get_utf8_index("RuntimeVisibleTypeAnnotations") as u16)
                    ...
                    .and(
                        table
                            .iter()
                            .fold(Ok(0), |_, x| self.write_type_annotation(x, cp)),
                    )
            }
            ...
            &Attribute::AnnotationDefault(ref value) => self
                .write_u16(cp.get_utf8_index("AnnotationDefault") as u16)
            ...
            &Attribute::BootstrapMethods(ref table) => {
                self.write_u16(cp.get_utf8_index("BootstrapMethods") as u16)
                    // attribute_length
                    .and(self.write_u32(table.iter().fold(2, |acc, method| {
                        acc + 4 + method.bootstrap_arguments.len() as u32 * 2
                    })))
                        ...
                        self.write_u16(method.bootstrap_method_ref.idx as u16)
                            ...
                            .and(
                                method
                                    .bootstrap_arguments
                                    .iter()
                                    .fold(Ok(0), |_, arg| self.write_u16(arg.idx as u16)),
                            )
                    }))
            }
    ...
    fn write_stack_map_table(
        ...
        self.write_u16(cp.get_utf8_index("StackMapTable") as u16)
            ...
            .and(
                self.write_u32(
                    2 + table.iter().map(|st| st.len()).fold(0, |acc, x| acc + x) as u32,
                ),
            )
            // number_of_entries
            .and(self.write_u16(table.len() as u16))
            ...
            .and(table.iter().fold(Ok(0), |_, x| {
                match x {
                    ...
                    &StackMapFrame::SameLocals1StackItemFrame { tag, ref stack } => {
                        self.write_u8(tag).and(self.write_verification_type(stack))
                    }
                    &StackMapFrame::SameLocals1StackItemFrameExtended {
                        ...
                        .and(self.write_verification_type(stack)),
                    &StackMapFrame::ChopFrame { tag, offset_delta } => {
                        self.write_u8(tag).and(self.write_u16(offset_delta))
                    }
                    &StackMapFrame::SameFrameExtended { offset_delta } => {
                        self.write_u8(251).and(self.write_u16(offset_delta))
                    }
                    &StackMapFrame::AppendFrame {
                        tag,
                        offset_delta,
                        ref locals,
                    } => self.write_u8(tag).and(self.write_u16(offset_delta)).and(
                        locals
                            .iter()
                            .fold(Ok(0), |_, x| self.write_verification_type(x)),
                    ),
    ...
    fn write_verification_type(&mut self, info: &VerificationType) -> Result<usize, Error> {
        match info {
            ...
            &VerificationType::Object { ref cpool_index } => {
                self.write_u8(7).and(self.write_u16(cpool_index.idx as u16))
            }
            &VerificationType::Uninitialized { offset } => {
                self.write_u8(8).and(self.write_u16(offset))
            }
        }
    }
    ...
    fn write_element_value(
        ...
        match element_value {
            &ElementValue::ConstantValue(tag, ref idx) => {
                self.write_u8(tag).and(self.write_u16(idx.idx as u16))
            }
            &ElementValue::Enum {
                ref type_name_index,
                ref const_name_index,
            } => self
                .write_u8(101)
                .and(self.write_u16(type_name_index.idx as u16))
                .and(self.write_u16(const_name_index.idx as u16)),
            &ElementValue::ClassInfo(ref idx) => {
                self.write_u8(99).and(self.write_u16(idx.idx as u16))
            }
            &ElementValue::Annotation(ref annotation) => {
                self.write_u8(64).and(self.write_annotation(annotation, cp))
            }
            &ElementValue::Array(ref table) => self
                .write_u8(91)
                .and(self.write_u16(table.len() as u16))
                .and(
                    table
                        .iter()
                        .fold(Ok(0), |_, x| self.write_element_value(x, cp)),
                ),
        }
    }

    fn write_element_value_pair(
        &mut self,
        pair: &ElementValuePair,
        cp: &ConstantPool,
    ) -> Result<usize, Error> {
        self.write_u16(pair.element_name_index.idx as u16)
            .and(self.write_element_value(&pair.value, cp))
    }
        ...
        self.write_u16(annotation.type_index.idx as u16)
            ...
            .and(
                annotation
                    .element_value_pairs
                    .iter()
                    .fold(Ok(0), |_, x| self.write_element_value_pair(x, cp)),
            )
    }
            ...
            .and({
                match &annotation.target_info {
                    ...
                    &TargetInfo::TypeParameterBound {
                        ...
                        param_idx,
                        bound_index,
                    } => self.write_u8(param_idx).and(self.write_u8(bound_index)),
                    ...
                    &TargetInfo::LocalVar {
                        subtype: _,
                        ref target,
                    } => self.write_u16(target.len() as u16).and(target.iter().fold(
                        Ok(0),
                        |_, x| {
                            self.write_u16(x.0)
                                .and(self.write_u16(x.1))
                                .and(self.write_u16(x.2))
                        },
                    )),
                    ...
                    &TargetInfo::TypeArgument {
                        ...
                        offset,
                        type_arg_idx,
                    } => self.write_u16(offset).and(self.write_u8(type_arg_idx)),
                }
            })
            .and({
                // path_length
                self.write_u8(annotation.target_path.path.len() as u8)
                    // path
                    .and(annotation.target_path.path.iter().fold(Ok(0), |_, x| {
                        self.write_u8(x.0.value()).and(self.write_u8(x.1))
                    }))
            })
    ...
    fn write_instructions(&mut self, instructions: &Vec<Instruction>) -> Result<usize, Error> {
        ...
        self.write_u32(target.len() as u32)
            .and_then(|x| self.write_n(&target).map(|y| x + y))
    }
    ...
    fn render_instruction(&mut self, instruction: &Instruction, offset: usize) -> usize {
        match instruction {
            ...
            &Instruction::CHECKCAST(value) => {
                self.write_u8(0xc0).and(self.write_u16(value)).and(Ok(3))
            }
            ...
            &Instruction::GETFIELD(value) => {
                self.write_u8(0xb4).and(self.write_u16(value)).and(Ok(3))
            }
            &Instruction::GETSTATIC(value) => {
                self.write_u8(0xb2).and(self.write_u16(value)).and(Ok(3))
            }
            &Instruction::GOTO(value) => self
                .write_u8(0xa7)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::GOTO_W(value) => self
                .write_u8(0xc8)
                .and(self.write_u32(value as u32))
                .and(Ok(5)),
            ...
            &Instruction::IF_ACMPEQ(value) => self
                .write_u8(0xa5)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IF_ACMPNE(value) => self
                .write_u8(0xa6)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IF_ICMPEQ(value) => self
                .write_u8(0x9f)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IF_ICMPNE(value) => self
                .write_u8(0xa0)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IF_ICMPLT(value) => self
                .write_u8(0xa1)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IF_ICMPGE(value) => self
                .write_u8(0xa2)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IF_ICMPGT(value) => self
                .write_u8(0xa3)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IF_ICMPLE(value) => self
                .write_u8(0xa4)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFEQ(value) => self
                .write_u8(0x99)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFNE(value) => self
                .write_u8(0x9a)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFLT(value) => self
                .write_u8(0x9b)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFGE(value) => self
                .write_u8(0x9c)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFGT(value) => self
                .write_u8(0x9d)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFLE(value) => self
                .write_u8(0x9e)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFNONNULL(value) => self
                .write_u8(0xc7)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IFNULL(value) => self
                .write_u8(0xc6)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::IINC(a, b) => self
                .write_u8(0x84)
                .and(self.write_u8(a))
                .and(self.write_u8(b as u8))
                .and(Ok(3)),
            ...
            &Instruction::INSTANCEOF(value) => {
                self.write_u8(0xc1).and(self.write_u16(value)).and(Ok(3))
            }
            &Instruction::INVOKEDYNAMIC(value) => self
                .write_u8(0xba)
                .and(self.write_u16(value))
                .and(self.write_u16(0))
                .and(Ok(5)),
            &Instruction::INVOKEINTERFACE(a, b) => self
                .write_u8(0xb9)
                .and(self.write_u16(a))
                .and(self.write_u8(b))
                .and(self.write_u8(0))
                .and(Ok(5)),
            &Instruction::INVOKESPECIAL(value) => {
                self.write_u8(0xb7).and(self.write_u16(value)).and(Ok(3))
            }
            &Instruction::INVOKESTATIC(value) => {
                self.write_u8(0xb8).and(self.write_u16(value)).and(Ok(3))
            }
            &Instruction::INVOKEVIRTUAL(value) => {
                self.write_u8(0xb6).and(self.write_u16(value)).and(Ok(3))
            }
            ...
            &Instruction::JSR(value) => self
                .write_u8(0xa8)
                .and(self.write_u16(value as u16))
                .and(Ok(3)),
            &Instruction::JSR_W(value) => self
                .write_u8(0xc9)
                .and(self.write_u32(value as u32))
                .and(Ok(5)),
            ...
            &Instruction::LDC2_W(value) => {
                self.write_u8(0x14).and(self.write_u16(value)).and(Ok(3))
            }
            ...
            &Instruction::MULTIANEWARRAY(a, b) => self
                .write_u8(0xc5)
                .and(self.write_u16(a))
                .and(self.write_u8(b))
                .and(Ok(4)),
            &Instruction::NEW(value) => self.write_u8(0xbb).and(self.write_u16(value)).and(Ok(3)),
            &Instruction::NEWARRAY(value) => {
                self.write_u8(0xbc).and(self.write_u8(value)).and(Ok(2))
            }
            &Instruction::NOP => self.write_u8(0x00),
            &Instruction::POP => self.write_u8(0x57),
            &Instruction::POP2 => self.write_u8(0x58),
            &Instruction::PUTFIELD(value) => {
                self.write_u8(0xb5).and(self.write_u16(value)).and(Ok(3))
            }
            &Instruction::PUTSTATIC(value) => {
                self.write_u8(0xb3).and(self.write_u16(value)).and(Ok(3))
            }
            ...
            &Instruction::SIPUSH(value) => {
                self.write_u8(0x11).and(self.write_u16(value)).and(Ok(3))
            }
            ...
            &Instruction::ILOAD_W(value) => {
                self.write_u16(0xc415).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::FLOAD_W(value) => {
                self.write_u16(0xc417).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::ALOAD_W(value) => {
                self.write_u16(0xc419).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::LLOAD_W(value) => {
                self.write_u16(0xc416).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::DLOAD_W(value) => {
                self.write_u16(0xc418).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::ISTORE_W(value) => {
                self.write_u16(0xc436).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::FSTORE_W(value) => {
                self.write_u16(0xc438).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::ASTORE_W(value) => {
                self.write_u16(0xc43a).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::LSTORE_W(value) => {
                self.write_u16(0xc437).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::DSTORE_W(value) => {
                self.write_u16(0xc439).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::RET_W(value) => {
                self.write_u16(0xc4a9).and(self.write_u16(value)).and(Ok(4))
            }
            &Instruction::IINC_W(a, b) => self
                .write_u16(0xc484)
                .and(self.write_u16(a))
                .and(self.write_u16(b as u16))
                .and(Ok(6)),
