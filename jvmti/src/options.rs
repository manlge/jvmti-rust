...
impl Options {
        ...
        if opt_args.len() > 0 {
            for arg in opt_args.split(",") {
                match arg.find('=') {
                    ...
                    None => {
                        Options::parse_directive(&mut options, arg);
                    }
                }
            }
        }
    ...
    fn parse_key_value(options: &mut Options, key: &str, value: &str) {
        match key {
            "agentid" => {
                options.agent_id = value.to_string();
            }
            "config" => {
                options.config_location = Some(value.to_string());
            }
            _ => {
                options
                    .custom_args
    ...
    fn parse_directive(options: &mut Options, directive: &str) {
        match directive {
            _ => options
                .custom_args
                .insert(directive.to_string(), "".to_string()),
        };
    }
